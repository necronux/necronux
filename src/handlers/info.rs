...
impl InfoSubCmd {
    #[trace_instrument(level = "info", name = "handle_info", skip(self, theme))]
    pub fn handle(&self, theme: &ThemedUi) -> Result<()> {
        ...
        let pb = task_msg!(
            ...
            wants_spinner: true,
            has_steps: false,
            ("Showing info...", theme::style::progress_task)
        )?;
        Self::info().context("Failed to show info")?;
        if let Some(pb) = pb {
            pb.finish();
        }

        {
            ...
            info!(
                ...
                "Successfully shown info"
            );
            result_success_msg!(
                theme,
                &mut stdout(),
                Some(json_obj!(
                    "info_elapsed" => elapsed_fmt.clone(),
                    "info_elapsed_secs" => elapsed.as_secs_f64(),
                )),
