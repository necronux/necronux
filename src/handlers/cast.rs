...
impl CastSubCmd {
    #[trace_instrument(level = "info", name = "handle_cast", skip(self, theme))]
    pub fn handle(&self, theme: &ThemedUi) -> Result<()> {
        info!("Handling cast request...");
...

        {
            ...
            info!(elapsed_secs = elapsed.as_secs_f64(), "Successfully cast");
            result_success_msg!(
                theme,
                &mut stdout(),
                Some(json_obj!(
                    "cast_elapsed" => elapsed_fmt.clone(),
                    "cast_elapsed_secs" => elapsed.as_secs_f64(),
                )),
