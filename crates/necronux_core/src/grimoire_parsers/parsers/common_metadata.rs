// ==-----------------------------------------------------------== //
// SPDX-FileCopyrightText: © 2025 Nayan Patil <nayantsg@proton.me>
//
// SPDX-License-Identifier: GPL-3.0-or-later
// ==-----------------------------------------------------------== //

use crate::{
    GrimoireParser,
    error::{ParseGrimoireError, ParseGrimoireErrorWithContext},
    grimoire_schemas::ParsedGrimoire,
};
use necronux_utils::trace_instrument;
use std::result as stdrt;
use tracing::debug;

#[derive(Debug)]
pub struct CommonMetadataParser;

impl GrimoireParser for CommonMetadataParser {
    fn name(&self) -> &str {
        "common metadata"
    }

    #[trace_instrument(level = "debug", name = "CommonMetadataParser::parse", skip(self))]
    fn parse(&self) -> stdrt::Result<ParsedGrimoire, ParseGrimoireErrorWithContext> {
        debug!(
            parser = %self.name(),
            "Parsing grimoire with {} parser...", self.name()
        );

        let parsed = parse_inner().map_err(|e| ParseGrimoireErrorWithContext { source: e })?;

        debug!(
            parser = %self.name(),
            "Successfully parsed grimoire with {} parser", self.name()
        );
        Ok(parsed)
    }
}

fn parse_inner() -> stdrt::Result<ParsedGrimoire, ParseGrimoireError> {
    let current_grimoire_dir = necronux_utils::paths::current_grimoire_path()?;
    let grimoire_info = necronux_pkg::CurrentGrimoireInfo::introspect()?;
    let grimoire_file = current_grimoire_dir
        .join(grimoire_info.package_name)
        .join("necronux.grimoire");

    let parsed = rpkl::from_config(&grimoire_file).map_err(|e| ParseGrimoireError::RpklError {
        path: grimoire_file.to_path_buf(),
        source: e,
    })?;
    Ok(ParsedGrimoire::CommonMetadata(parsed))
}
