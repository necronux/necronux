...
impl TryFrom<NormalizedToolProvisioningCommands> for ValidatedToolProvisioningCommands {
    ...
    fn try_from(s: NormalizedToolProvisioningCommands) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            install_command_prefix_args: s
                .install_command_prefix_args
                .map(|i| {
                    validators::ensure_str_is_not_empty(
                        i,
            ...
            verify_installation_command_prefix_args: s
                .verify_installation_command_prefix_args
                .map(|v| {
                    validators::ensure_str_is_not_empty(
                        v,
                        "verifyInstallationCommandPrefixArgs",
                        parent_object,
                    )
                })
                .transpose()?,
            verify_installation_command: validators::ensure_str_is_not_empty(
                s.verify_installation_command,
                "verifyInstallationCommand",
                parent_object,
            )?,
            uninstall_command_prefix_args: s
                .uninstall_command_prefix_args
                .map(|u| {
                    validators::ensure_str_is_not_empty(
                        u,
